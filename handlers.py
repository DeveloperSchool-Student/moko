...
from models import IPO, CollectionCard, IPOApplication, UserCollection
...
from models import TycoonBattle, PlayerBet
...
async def cmd_add_stock(message: types.Message):
    ...
    try:
        ...
        async with async_session() as session:
            ...
            await session.flush()
            # --- СТВОРЕННЯ КАРТ ДЛЯ НОВОЇ АКЦІЇ ---
        ...
        await message.answer(
            f"✅ <b>Акцію додано!</b>\n"
            f"🏷 {ticker}\n"
            f"🎴 Колекційні карти згенеровано!", 
            parse_mode="HTML"
        )
...
async def cb_confirm_reset(callback: types.CallbackQuery):
    ...
    await callback.message.edit_text("⏳ <b>Виконую повне очищення...</b>", parse_mode="HTML")

    async with async_session() as session:
        try:
            # 1. Спочатку розриваємо зв'язок користувачів з кланами (щоб не було помилок FK)
            await session.execute(update(User).values(clan_id=None))
            
            # 2. Видаляємо всі залежні дані (дочірні таблиці)
            await session.execute(delete(IPOApplication))
            await session.execute(delete(PlayerBet))
            await session.execute(delete(TycoonBattle))
            await session.execute(delete(UsedPromo))
            ...
            await callback.message.edit_text(
                "💣 <b>Світ успішно скинуто!</b>\n\n"
                "Всі дані видалено. Перезапустіть бота командою /start", 
                parse_mode="HTML"
            )
        except Exception as e:
            await session.rollback()
            await callback.message.edit_text(f"❌ Помилка при скиданні: {e}")
